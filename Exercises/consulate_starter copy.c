...
bool enqueue(Data *data, char *name)
{
    ...
    if(data->first == NULL){
        data->first = new;
        printf("New Queue Head %s\n",data->first->name);
        return true;
    }
    if(data->last == NULL){
        data->last = new;
        if(data->first->next == NULL){
            printf("----LINK HEAD TO %s---\t",data->last->name);
            data->first->next = data->last;
        }
        printf("---New Queue Item %s ---  \n",data->last->name);
        return true;
    }
    new->next = data->last;
    data->last = new;
...
bool dequeue(Data *data, char *returned_name)
{
    ...
    if(data->first == NULL){
        printf("Failed from dequeue as *first is null\n");
        return false;
    }
    printf("Current Queue Head %s\n",data->first->name);
