...
note *BST_createNewNodes(BST_Node *root,int semitones,double time_shift,note* shifted){

    if(root == NULL){return shifted;} // Return shifted without new additions if root is null
    if(root->left != NULL){
        /// Recusrive Traversal of Left Sub Tree
        shifted = BST_createNewNodes(root->left,semitones,time_shift,shifted);
    }
    if(root != NULL) {
        // Visit Operation
       double newFreq = newNodeFreq(root->freq,semitones);
       // Fetch Frequency according to the requirements
       ...
       while (BST_search(root,newBar,newIndex) != NULL)
       {
            newIndex += 1e-05;
       }
       
        // Adding a new Note to shifted
       shifted = playlist_insert(shifted,newFreq,newBar,newIndex);

    }   
    if(root->right != NULL){
        // Recusrive Traversal of Right Sub Tree
        shifted = BST_createNewNodes(root->right,semitones,time_shift,shifted);  
    }

    return shifted; // Returning Shifted Note Class a Linked List of Shifted Notes
}

BST_Node *BST_InsertNewNodes(note* shifted, BST_Node *root){
   while (shifted != NULL)
   {
       // Linked List Traversal of shifted
       BST_Node *node = newBST_Node(shifted->freq,shifted->bar,shifted->index);
...
BST_Node *BST_harmonize(BST_Node *root, int semitones, double time_shift)
{
    ...
    if(root == NULL){return NULL;}
    note* shifted;
    shifted = BST_createNewNodes(root,semitones,time_shift,NULL);
    // printf("--- New Nodes -- \n");
    ...
    //return root;  
    return BST_InsertNewNodes(shifted,root);
}
